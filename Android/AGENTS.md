...
- The detail screen (`PostDetailScreen`/`PostDetailViewModel`) fetches the real document and renders it — Leaflet blocks via the pre-existing `LeafletBlockRenderer`, Markpub markdown via `MarkdownRendererView` with full block and inline formatting, legacy `textContent` as plain paragraphs, and a generic plaintext-leaf walk for any other/unmodelled block-array format so content isn't silently dropped. Rich-text facets (bold/links inside Leaflet text/paragraph blocks) are still not rendered. Comments and interactions (likes, reposts, replies) are implemented.
