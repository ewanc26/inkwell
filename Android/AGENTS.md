...
- App-level test coverage is two files and no instrumentation tests: `StandardSiteVerifierTest` (13 hermetic tests) and `SearchModelsTest` (3 tests). `./gradlew test` otherwise executes effectively empty tasks for the rest of the app; never report that as behavioral coverage beyond these two files.
