...
- Read `README.md`, Gradle/version catalog files, `AndroidManifest.xml`, `docs/oauth/client-metadata.json`, and all touched Kotlin. Compare shared wire behavior with the owned iOS `../iOS/` checkout, without copying Swift lifecycle or security assumptions. The `shared/` KMP module provides `MarkdownParser`, `FacetConverter`, `FacetSchema`, `ReaderTheme`, `TipPromptPolicy`, `NotificationPolicy`, `VerificationUrls`, `DocumentLinkScanner`, `ConstellationPagination`, `UrlUtils`, and neutral model types (`BlobRef`, `StrongRef`, `RgbColor`, `BasicTheme`, `PublicationTheme`, etc.); prefer shared logic over local copies.
