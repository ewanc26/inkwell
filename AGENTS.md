...
- **iOS:** `iOS/` is authoritative; `.letta/worktrees/` contains local shadow checkouts and must not be edited as product source.
  ...
  - `iOS/Inkwell/Features/` owns Read/Discover/Write and background subscription polling. `iOS/InkwellTests/StandardSiteTests.swift` is a focused nine-test unit suite, not end-to-end OAuth/editor/rendering coverage.
