...
export const enGB = {
  ...
  meta: {
    ...
    "/security": {
      ...
      description:
        "How Inkwell handles authentication, key storage, and verification: OAuth 2.1 with DPoP-bound tokens, no app passwords, on-device Keychain and Android Keystore storage, and no analytics or tracking.",
    },
    "/about": {
      title: "Inkwell — About",
      description:
    ...
    compare: {
      ...
      rows: [
        ...
        },
        {
          ...
          app: "Held in the Keychain or the Android Keystore",
        },
        {
  ...
  security: {
    ...
    cards: {
      ...
      storage: {
        ...
        body: "Your OAuth session and DPoP private key are held in the platform's secure storage, not app-readable preferences: Apple's Keychain on iOS, and on Android an AES-256-GCM envelope sealed under a non-exportable key that lives in the Android Keystore, hardware-backed where the device has a secure element. Android excludes the OAuth session from backup and device transfer because the Keystore key cannot be restored safely. Signing out or uninstalling removes them. Full detail — including backup behaviour on each platform — is in the [Privacy Policy](privacy).",
      },
      analytics: {
