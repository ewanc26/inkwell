...
- `src/routes/[[locale=locale]]/+page.svelte` is the landing page; `/privacy` and `/terms` are substantive legal promises; `/client-metadata.json` is a live OAuth client identity consumed by PDS servers. `src/routes/+layout.svelte` sits above the locale group and owns metadata/navigation/footer and client-side mobile navigation.
...
- `/privacy` and `/terms` are generated from `../legal/*.md` (the same sources compiled into the shared KMP module's `LegalDocuments.kt`, which iOS and Android render natively in-app) via `node tools/legal/render.mjs`, which writes `src/lib/legal/documents.ts` — one rendering per document per locale, each carrying its provenance. Never hand-edit that file: edit the Markdown source and regenerate. The route components (`src/routes/[[locale=locale]]/{privacy,terms}/+page.svelte`) are thin wrappers around `src/lib/components/LegalDocument.svelte`, which renders the generated body plus the heading, version line, and translation notice from the message catalogue — so a translated legal page has no hard-coded strings in the generator. Keep platform-specific storage and third-party disclosures in the source accurate; see `../iOS/AGENTS.md` and `../Android/AGENTS.md` for platform-specific storage details.
- **A translated legal page must never show older wording under the current effective date.** `../legal/meta.json` records, per locale, the source version each translation was made from, when it was reviewed, and a digest of the source at that review. When the source moves on, the translation is stale: `LegalDocument.svelte` drops the effective date entirely, states which version the wording means and which is actually in force, and links the authoritative British English text — and `node tools/legal/render.mjs --check` (CI's `legal-sync` job) fails. Update the translated source and run `node tools/legal/render.mjs --bless <locale>` in the same change as any `legal/*.md` edit.
...
- Preview **every locale**, not just the source one: confirm `<html lang>`/`Content-Language`, `<title>`, canonical, `hreflang` alternates and `og:locale` change with the prefix (`/` vs `/fr`), that `/client-metadata.json` is byte-identical and unprefixed, and that a translated legal page carries its provenance notice.
