...
- The Terms claim native mute/block/report tools. Mute/block management now has user-facing evidence on both platforms; reporting does not. Do not treat `com.atproto.moderation.report` as implemented or expand the reporting promise without an actual client flow. Legal copy, App Store status, and “available” wording require evidence, not roadmap intent.
