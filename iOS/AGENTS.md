...
- `InkwellTests/StandardSiteTests.swift` is a focused nine-test unit suite, not end-to-end OAuth/editor/rendering coverage. It is also the *only* test source in this directory, and it does not exercise the shared KMP core behind `SharedKMP.swift` — those tests live in `../shared/src/commonTest/` and run through Gradle.
...
- Unit tests cover the Inkwell NSID namespace, AT-URI rejection of malformed values, association/canonical URLs, verification endpoint paths, wire keys, search v2 decoding, notification JSON, and tolerant record pages — nine tests in `InkwellTests/StandardSiteTests.swift`, the target's only source file.
