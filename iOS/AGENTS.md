...
- `Inkwell/SharedKMP.swift` is the thin Swift wrapper around the Kotlin Multiplatform shared core (`InkwellShared.xcframework`), exposing `parseAtUri`, `parseMarkdown`, `serializeMarkdown`, `resolveReaderTheme`, `shouldShowTip`, and notification policy helpers.
- `Inkwell/Rendering/` handles Markpub Markdown, Leaflet block/blob pages, pckt, Offprint, Bluesky embeds, polls, and themes. Markdown parsing and serialization are delegated to the shared KMP via `SharedKMP.swift`; the native `MarkdownParser`/`MarkdownSerializer` in `ContentProvider.swift` remain as the editor's round-trip implementation.
