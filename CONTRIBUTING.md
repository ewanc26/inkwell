...
Use one unified GitHub Release per app version, tagged `vX.Y.Z`. Attach the
platform artifacts that exist — `Inkwell-X.Y.Z.ipa` for iOS and
`Inkwell-X.Y.Z.apk` for Android — and separate iOS and Android release notes
with headings when their changes differ. Use the latest relevant release
commit as the tag target. The historical `ios-vX.Y.Z` and `android-vX.Y.Z`
tags remain available for provenance, but new releases must not create
separate platform-specific GitHub Release records.
